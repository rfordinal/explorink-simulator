...
| `src/HalDisplay.cpp` | A software-renderer fallback when `SDL_CreateRenderer` finds no accelerated driver. A headless run (`SDL_VIDEODRIVER=dummy`, which is how a scripted screenshot run avoids stealing focus) left `sdl_renderer` null, and `presentIfNeeded()` returns early on null -- so the process ran its whole input script, exited 0, wrote no BMP and printed no error. Verified 2026-09-06: with the fallback, a dummy-driver run writes the same map frame a windowed run does. Upstream material. |
...
| `src/HalGPIO.h/.cpp` | `updateSequence()`, a counter the firmware reads to do work once per input frame (`MappedInputManager` pumps the touch hint boxes off it). ExplorInk's own `lib/hal/HalGPIO.h` ticks it in `update()`; here it ticks in `beginFrame()`, because the firmware calls `update()` several times inside one simulator frame and a per-`update()` tick would re-pump touch edges `beginFrame()` has not cleared yet. |
| `src/BoardConfig.h` | `BoardProfile::displayWidth` / `displayHeight`, the landscape framebuffer pair freeink-sdk's own `BoardProfile` carries. ExplorInk's `HintGeometry` reads it directly to derive the portrait panel size, so the reduced profile here has to carry it too. The numbers must agree with `EInkDisplay`'s compile-time `DISPLAY_WIDTH`/`DISPLAY_HEIGHT`, which is why `PAPER_MONO` is 800x480 here: the simulator has no `SIMULATOR_DEVICE_PAPERMONO` branch in `EInkDisplay.h` and falls through to the 800x480 default. |
